...
where scores are averaged across all datasets in each domain and output formats for each LM.
'''
import os
cwd = os.getcwd()
...
if __name__ == "__main__":
	...
	domains = ["MWP", "MHQA", "Plan.", "Relation."]
	domain2dataset = {
		"MWP": ["ASDIV", "GSM8K", "MULTIARITH", "SVAMP"],
		"MHQA": ["DATE", "STRATEGYQA", "SPORT"],
		"Plan.": ["SAYCAN"],
		"Relation.": ["CLUTRR"]
	}
	...
	for val in vals:
		...
		for LM in LMs:
			...
			for output_format in output_formats:
				...
				for confidence_metric in confidence_metrics:
					for domain in domains:
						domain_result = np.mean([results[confidence_metric][(val, domain_dataset)] for domain_dataset in domain2dataset[domain]])
						LM_results[domain][confidence_metric][output_format] = "{:.3f}".format(domain_result)
			...
			for domain in domains + ["average"]:
				for confidence_metric in confidence_metrics:
					all_results[LM][domain][confidence_metric] = "{:.3f}".format(np.mean([float(v) for v in LM_results[domain][confidence_metric].values()]))
		# save results
		output_dir = f"score_dir/calibration_error/all_LMs"
		if not os.path.exists(output_dir):
			os.makedirs(output_dir)
		fwn = f"{output_dir}/calerror_across_methods_{val}_wdomain.csv"
